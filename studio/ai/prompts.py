...
- `toast.success(msg)` / `toast.error(msg)`; `getIcon(name)` (returns the "lucide-<name>" class; prefer writing "lucide-<name>" directly); variables (refs — read/write via `.value`); `route`, `router`.
